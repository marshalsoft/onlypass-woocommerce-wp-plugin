...
class WC_Gateway_OnlyPass extends WC_Payment_Gateway {
    ...
    public function __construct() {
        $this->baseUrl = "https://devapi.onlypassafrica.com/api/v1/external/payments";
        ...
        $onlypass_api_key = get_option('currency_type');
        ...
        if(isset($_POST["woocommerce_onlypass_api_key"]) && isset($_POST["woocommerce_onlypass_merchent_id"])) {
            ...
            $response = wp_remote_post($this->baseUrl . "/channels", array(
                'method' => 'POST',
                'timeout' => 45,
                'redirection' => 5,
                'httpversion' => '1.0',
            ...
            if (is_wp_error($response)) {
                ...
                if (!$json_values["status"]) {
                  ...
                  $this->gateways = bin2hex(json_encode($json_values["data"]));
                  $currency_type = $this->get_option('currency_type' );
                  if($onlypass_api_key == null)
                  {
                      add_option('onlypass_api_key',$apikey);
                      add_option('onlypass_merchent_id',$merchantId);
                  }else{
                      update_option('onlypass_api_key',$apikey);
                      update_option('onlypass_merchent_id',$merchantId);
                  }

                  if($gateways == null)
                  {
                      add_option('onlypass_gateways',$this->gateways);
                  }else{
                      update_option('onlypass_gateways',$this->gateways);
                  }
                    if($currency_type == null)
                    {
                        add_option('onlypass_currency_type',$currency);
                    }else{
                        update_option('onlypass_currency_type',$currency_type);
                    }
        ...
        $this->gateways  =  get_option( 'onlypass_gateways' );
